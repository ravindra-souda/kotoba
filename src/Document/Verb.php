...
#[ApiResource(
    ...
    operations: [
        ...
        new Delete(),
        new Put(
            controller: FetchVerbByCode::class,
            uriTemplate: '/verbs/{code}',
...
class Verb extends Card
{
    ...
    public function hasValidGroup(
        ...
        /** @var array{dictionary:string} $inflections */
    ...
    public function hasValidInflections(
        ...
        if (empty($inflections['dictionary'])
            || !is_string($inflections['dictionary'])) {
            return false;
        }
    ...
    private function fillRomaji(): static
    {
        ...
        $romaji = $this->toRomaji($this->hiragana ?? $katakanaString);
        if (!$romaji) {
            throw new \Exception('Error getting romaji from hiragana/katakana');
        }
        $this->romaji ??= $romaji;
