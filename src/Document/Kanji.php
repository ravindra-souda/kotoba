...
class Kanji extends Card
{
    ...
    {
        if (null === $this->kunyomi) {
            return $this;
        }

        array_walk($this->kunyomi, function (&$value) {
            $value = $this->toHiragana($value);
            if (!$value) {
                throw new \Exception('Error converting kunyomi to hiragana');
            }
        });
    ...
    private function fillOnyomi(): static
    {
        ...
        array_walk($this->onyomi, function (&$value) {
            $value = $this->toKatakana($value, false);
            if (!$value) {
                throw new \Exception('Error converting onyomi to katakana');
            }
        });
